...
fetch_rootfs() {
	...
	curl -L --fail --ftp-pasv --retry 3 --retry-delay 3 -o $TARBALLIMG.sha512sum $sha
	sha512sum $TARBALLIMG | awk '{print $1}' > $TARBALLIMG.checksum
	sed -i -r 's/(\s+)?\S+//2' $TARBALLIMG.sha512sum
	diff $TARBALLIMG.sha512sum $TARBALLIMG.checksum || die "Checksum error, fetch de tarball again"
	# cleanup
	rm -r $TARBALLIMG.checksum
}
...
compress_rootfs() {
	...
	XZ_DEFAULTS='-T0' tar --exclude="var/cache/scratchpkg/packages/*" \
		--exclude="var/cache/scratchpkg/repos/*" \
		...
		if [ "$?" != 0 ]; then
			...
			cd "$PORTSDIR"
			sha512sum $(basename $(realpath "$TARBALLIMG")) > $(basename $(realpath "$TARBALLIMG")).sha512sum
			rm -f "$TARBALLIMG".bak
		fi
	cd ..
}
...
make_iso() {
	...
	mksquashfs "$ROOTFS" "$ISODIR/rootfs/filesystem.sfs" \
			...
			-e "$ROOTFS"/var/cache/scratchpkg/repos/* \
			-e "$ROOTFS"/usr/ports/* \
			-e "$ROOTFS"/root/* \
			-e "$ROOTFS"/tmp/* \
	...
	rm -f "$OUTPUTISO" "$OUTPUTISO.sha512sum"
	xorriso -as mkisofs \
...
ARCH=$(uname -m)
TARBALLIMG="$PORTSDIR/venomlinux-rootfs-$INIT-$ARCH.tar.xz"
...
OUTPUTISO="${OUTPUTISO:-$PORTSDIR/venomlinux-$INIT-$(date +%Y%m%d)-$ARCH.iso}"
